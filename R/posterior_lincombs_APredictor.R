## Distribución a posteriori del intercept ##
lc1 = inla.make.lincomb(APredictor = rep(1/(S*T),S*T))
names(lc1) <- "intercept"


## Distribución a posteriori del patrón espacial ##
M1 <- matrix(-1/(S*T),S,S*T)
for (i in 1:S) {
	M1[i,seq(0,S*(T-1),by=S)+i] <- 1/(S*T)*(S-1)
}
lc2 = inla.make.lincombs(APredictor = M1)
names(lc2) <- paste("spatial.",as.character(seq(1:S)),sep="")


## Distribución a posteriori del patrón temporal ##
M2 <- matrix(-1/(S*T),T,S*T)
for (i in 1:T) {
	M2[i,seq(1,S)+S*(i-1)] <- 1/(S*T)*(T-1)
}
lc3 = inla.make.lincombs(APredictor = M2)
names(lc3) <- paste("temporal.",as.character(seq(1:T)),sep="")


## Distribución a posteriori del patrón espacio-temporal ##
...
lc4 = inla.make.lincombs(APredictor = M3)
